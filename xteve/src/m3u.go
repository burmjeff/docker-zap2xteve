...
func buildM3U(groups []string) (m3u string, err error) {
	...
	for _, channelNumber := range channelNumbers {
		...
		var parameter = fmt.Sprintf(`#EXTINF:0 timeshift="%s" channelID="%s" tvg-chno="%s" tvg-name="%s" tvg-id="%s" tvg-logo="%s" group-title="%s",%s`+"\n", channel.XTimeshift, channel.XEPG, channel.XChannelID, channel.XName, channel.XChannelID, imgc.Image.GetURL(channel.TvgLogo), channel.XGroupTitle, channel.XName)
